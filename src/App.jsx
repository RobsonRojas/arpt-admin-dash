...
import { 
  ...
  Forest, Warning, Rule, WhatsApp, Lock, Logout, HomeWork, Edit
...
const INITIAL_PROJECTS = [
  { id: "PROJ-01", status: "Em Análise", descricao: "Manejo Pirarucu", proponente: "Assoc. Solimões", municipio: "Tefé", tamanho: 1200, custo: 85000, risco: "Baixo" },
  { id: "PROJ-02", status: "Aprovado", descricao: "Agrofloresta Cacau", proponente: "Coop. Verde", municipio: "Altamira", tamanho: 450, custo: 120000, risco: "Baixo" }
];
...
const INITIAL_PROPS = [
  { id: "PROP-101", proprietario: "Maria Graças", car: "AM-1302401", municipio: "Tefé", area: 50, status: "Regular", lat: -3.35, lng: -64.71 },
  { id: "PROP-102", proprietario: "José Ferreira", car: "AM-9988771", municipio: "Alvarães", area: 120, status: "Pendente", lat: -3.22, lng: -64.80 }
];

const THEME = createTheme({
  palette: { primary: { main: '#1b5e20' }, secondary: { main: '#4caf50' }, background: { default: '#f4f6f8' } },
  components: { MuiButton: { styleOverrides: { root: { borderRadius: 8, textTransform: 'none' } } }, MuiPaper: { styleOverrides: { rounded: { borderRadius: 12 } } } }
});

// --- HELPER COMPONENTS ---
const MapEmbed = ({ lat, lng }) => {
  const isError = lat > 5 || lat < -15 || lng > -40 || lng < -75;
  if (isError) return <Box sx={{ height: 200, bgcolor: '#ffebee', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid red' }}><Typography color="error">ALERTA GEOFENCE: Fora da Amazônia</Typography></Box>;
  return <Box sx={{ height: 200, bgcolor: '#eee' }}><iframe width="100%" height="100%" frameBorder="0" src={`https://www.openstreetmap.org/export/embed.html?bbox=${lng-0.05},${lat-0.05},${lng+0.05},${lat+0.05}&layer=mapnik&marker=${lat},${lng}`} /></Box>;
};
...
const PropertiesView = () => {
  ...
  return (
    <Box sx={{ animation: 'fadeIn 0.5s' }}>
      <Box display="flex" justifyContent="space-between" mb={3}><Typography variant="h5"><HomeWork /> Propriedades</Typography><Button variant="contained" startIcon={<Add />} onClick={()=>{setForm({});setEditing(false);setOpen(true)}}>Nova Propriedade</Button></Box>
      ...
      <Dialog open={open} onClose={()=>setOpen(false)} fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} mt={1}>
            ...
            <Grid item xs={12}><Button variant="outlined" component="label" startIcon={<CloudUpload />}>Upload Foto CAR<input hidden accept="image/*" type="file" /></Button></Grid>
          </Grid>
        </DialogContent>
      ...
      <Drawer anchor="right" open={Boolean(selected)} onClose={()=>setSelected(null)}><Box p={3} width={350}><Typography variant="h6">Detalhes</Typography>{selected && <><Typography><b>{selected.proprietario}</b></Typography><Typography variant="body2">{selected.municipio}</Typography><Divider sx={{my:2}}/><MapEmbed lat={selected.lat || -3.0} lng={selected.lng || -60.0} /></>}</Box></Drawer>
    </Box>
  );
};
...
const ProjectsView = () => (
  <Box><Typography variant="h5" mb={2}>Projetos de Manejo</Typography><TableContainer component={Paper}><Table><TableHead><TableRow><TableCell>Projeto</TableCell><TableCell>Status</TableCell><TableCell>Custo</TableCell></TableRow></TableHead><TableBody>{INITIAL_PROJECTS.map(p=><TableRow key={p.id}><TableCell>{p.descricao}</TableCell><TableCell><Chip label={p.status} color={p.status==='Aprovado'?'success':'warning'} size="small"/></TableCell><TableCell>R$ {p.custo}</TableCell></TableRow>)}</TableBody></Table></TableContainer></Box>
);

const SponsorsView = () => (<Box><Typography variant="h5">Patrocinadores</Typography><Alert severity="info">Módulo RWA ativo.</Alert></Box>);
...
export default function App() {
  ...
  return (
    <ThemeProvider theme={THEME}>
      <CssBaseline />
      <AppBar position="fixed" sx={{zIndex:1201}}><Toolbar><IconButton color="inherit" onClick={()=>setDrawer(!drawer)} sx={{mr:2}}><MenuIcon/></IconButton><Typography variant="h6" flexGrow={1}>ARPT Admin</Typography><IconButton color="inherit" onClick={()=>signOut(auth)}><Logout/></IconButton></Toolbar></AppBar>
      <Box display="flex">
